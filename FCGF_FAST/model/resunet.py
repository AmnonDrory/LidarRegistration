...
class FCGF_net(nn.Module):
    def __init__(self, device):
        super(FCGF_net, self).__init__()
        self.device = device
        self.Model = ResUNetBN2C(1,
              32,
              bn_momentum=0.05,
              conv1_kernel_size=5,
    ...
    def prep_input_batch(self, PC):

        iC = PC.int()
        iF = torch.ones([iC.shape[0], 1], dtype=torch.float, device=iC.device)
        return iC, iF

    def forward(self, PC):
        iC, iF = self.prep_input_batch(PC)
        sinput0 = ME.SparseTensor(iF, iC)
        feature = self.Model(sinput0).F
        assert (feature.shape[0]==iC.shape[0]), "Error: FCGF network was given non-unique coordinates!"
        return feature
