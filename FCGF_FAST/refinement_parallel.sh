...
for i in ${!gpu_inds[@]}; do
  CUDA_VISIBLE_DEVICES=${gpu_inds[$i]} python -m refinement test_parallel $start_time $file_base $n_gpus ${i} "$@" &
done

wait < <(jobs -p)

python -m refinement test_parallel $start_time $file_base $n_gpus analysis "$@"
